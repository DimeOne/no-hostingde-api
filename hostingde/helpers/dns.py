...
def getRecordToAddEntry(name, type, content, ttl=8600):
    return {"name": name, "type": type, "content": content, "ttl": ttl}
...
def getZonesOrderedByDepth(zones):
    return sorted(
        zones, key=lambda x: (x["zoneConfig"]["nameUnicode"].count(".")), reverse=True
    )


def zoneRecordMatches(record, recordName, recordType=None, recordContent=None):
    if (
        record["name"].lower() == recordName.lower()
        and (recordType is None or record["type"].lower() == recordType.lower())
        and (
            recordContent is None or record["content"].lower() == recordContent.lower()
        )
    ):
        return True
    return False
...
def getZoneUpdateFromZone(
    ...
    previousRecords = getMatchingRecordsFromZone(
        zone, recordName, recordType, oldContent
    )
